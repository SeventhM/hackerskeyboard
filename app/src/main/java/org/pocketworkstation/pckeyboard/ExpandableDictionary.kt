 ...
 */
open class ExpandableDictionary internal constructor(val mContext: Context, dicTypeId: Int) :
    Dictionary() {
    private val mWordBuilder = CharArray(MAX_WORD_LENGTH)
    ...
    class NextWord(var word: Node?, var frequency: Int) {
        var nextWord: NextWord? = null
    }

    private var mRoots: NodeArray? = null
    private var mCodes: Array<IntArray?>
    fun loadDictionary() {
        synchronized(mUpdatingLock) { startDictionaryLoadingTaskLocked() }
    }
    ...
    open fun startDictionaryLoadingTaskLocked() {
        if (!mUpdatingDictionary) {
            ...
            executor.execute {
                loadDictionaryAsync()
                synchronized(mUpdatingLock) { mUpdatingDictionary = false }
            }
        }
    }

    var requiresReload: Boolean
        get() = mRequiresReload
        set(reload) {
            synchronized(mUpdatingLock) { mRequiresReload = reload }
        }
    ...
    private fun addWordRec(
        children: NodeArray?, word: String, depth: Int,
        frequency: Int, parentNode: Node?
    ) {
        ...
        if (wordLength == depth + 1) {
            ...
            childNode.frequency = max(frequency, childNode.frequency)
     ...
     */
    protected fun getWordsRec(
            roots: NodeArray?, codes: WordComposer?, word: CharArray,
            depth: Int, completion: Boolean, snr: Int, inputIndex: Int, skipPos: Int,
        ...
        for (i in 0 until count) {
            ...
            if (completion) {
                ...
                if (children != null) {
                    getWordsRec(
                        children, codes, word, depth + 1, true, snr, inputIndex,
                        skipPos, callback
                    )
                }
                ...
                if (children != null) {
                    getWordsRec(
                        children, codes, word, depth + 1, false, snr, inputIndex,
                        skipPos, callback
                    )
                }
            } else {
    ...
    private fun addOrSetBigram(
        ...
        val bigram = firstWord!!.ngrams
        if (bigram.isNullOrEmpty()) {
    ...
    private fun reverseLookUp(
        ...
        for (nextWord in terminalNodes!!) {
            ...
            if (freq >= UserBigramDictionary.SUGGEST_THRESHOLD) {
                ...
                callback!!.addWord(
                    sb.toString().toCharArray(), 0, sb.length, freq, mDicTypeId,
                    DataType.BIGRAM
                )
            }
        }
    ...
    private fun searchNode(
        children: NodeArray?, word: CharSequence?, offset: Int,
        length: Int
    ): Node? {
            ...
            if (node!!.code == currentChar) {
                if (offset == length - 1) {
                    ...
                    if (node.children != null) {
                        val returnNode = searchNode(
                            node.children, word, offset + 1, length
                        )
    ...
    init {
        clearDictionary()
        mCodes = arrayOfNulls(MAX_WORD_LENGTH)
    ...
    companion object {
        ...
        fun toLowerCase(character: Char): Char {
            var c = character
            if (c.code < BASE_CHARS.size) {
                c = BASE_CHARS[c.code]
            }
            if (c in 'A'..'Z') {
                c = (c.code or 32).toChar()
            } else if (c.code > 127) {
                c = c.lowercaseChar()
            }
            return c
